...
import { getNextInvoiceNumber, addInvoice, getInvoiceById, updateInvoice } from '../../services/taxInvoiceApi';
...
const EditInvoice = () => {
  ...
  // Invoice fetch for edit
  useEffect(() => {
    if (id) {
      getInvoiceById(id).then(data => {
        // yahan data se form fields set karo, jaise:
        setBillTo({ title: data.buyer_name, address: data.buyer_address, gst: data.buyer_gst });
        setShipTo({ title: data.consignee_name, address: data.consignee_address, gst: data.consignee_gst });
        setInvoiceNumber(data.invoice_number);
        setDate(data.invoice_date);
        setDeliveryNote(data.delivery_note);
        setModeOfPayment(data.payment_mode);
        setDeliveryNoteDate(data.delivery_note_date);
        setDestination(data.destination);
        setTermsOfDelivery(data.terms_to_delivery);
        setSelectedCountry({ name: data.country, symbol: data.currency_symbol, code: data.currency });
  ...
  // Save button par updateInvoice call karo
  // const handleSaveInvoice = async () => {
  //   // ...form validation...
  //   const invoiceData = {
  //     // ...form fields...
