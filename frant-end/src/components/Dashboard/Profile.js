...
const Profile = () => {
  ...
  const handleVerifyNewEmailOtp = async () => {
    ...
    try {
      ...
      if (res.success) {
        // Now update email
        const updateRes = await updateEmail(token, newEmail);
        if (updateRes.success) {
          ...
          setNewEmailOtpError(updateRes.message || "Failed to update email");
          toast.error(updateRes.message || "Failed to update email");
        }
      } else {
